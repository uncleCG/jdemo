...
public class JdController {
    ...
    public String postOneController(@RequestBody User user, HttpServletResponse response, HttpServletRequest request) throws JsonProcessingException {
        log.info("postOneController - return -> {}", user);
        response.addHeader("headerKey1", "headerVal1");
        response.addHeader("headerKey2", "headerVal2");
        Enumeration<String> headerNames = request.getHeaderNames();
        while (headerNames.hasMoreElements()) {
            String headerName = headerNames.nextElement();
            log.info("postOneController get header:{}={}", headerName, request.getHeader(headerName));
        }
